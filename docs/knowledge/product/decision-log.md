...
## D-026 · 下一阶段采用个人开发者本地单节点产品形态

- **日期**：2026-09-17
- **决策**：下一阶段以个人开发者 / Coding Agent 用户为首要用户；CodeWiki 在一台开发机上完整运行，只有一套知识内核和一份权威数据，Local Web、CLI、MCP 共享应用服务。Canonical Knowledge、Evidence 与 Human Overlay 是权威资产；SQLite FTS、缓存、页面及未来可能的向量索引均为可重建投影。默认不引入向量数据库，也不建设云服务、本地—云端同步、多租户或组织级多仓搜索。
- **背景**：用户当前没有云服务器，希望小规模代码库可直接在本机处理；此前“本地引擎 + 控制面”和中心化服务方案会提前引入同步、权限与运维复杂度。
- **来源**：[2026-09-17 本地 Beta 设计定稿](../../design/2026-09-17-local-beta-design.html)；根 README“下一阶段”章节。
- **状态**：effective（取代下一阶段中心化/混合部署讨论，不改变未来可部署同一内核的可能性）。

## D-027 · 重大设计定稿必须同步 README 与文档库

- **日期**：2026-09-17
- **决策**：每次重大产品、架构或路线设计获得用户确认后，必须在同一变更中完成四件事：归档设计原件到 `docs/`；更新根 `README.md` 的产品现状与路线；更新 `docs/README.md` 索引和状态；把长期有效取舍写入本决策日志。四处不一致时，设计视为尚未完成。
- **背景**：设计内容如果只留在会话或临时 HTML 中，GitHub 访问者和后续 Agent 无法获得同一上下文；README 是项目对外入口，文档库是可审计事实基础，两者必须同步。
- **来源**：用户 2026-09-17 明确要求“以后每次大规模设计定稿，就更新项目文档和 README，并作为底层铁律”；`AGENTS.md` 项目工作流。
- **状态**：effective。

## D-028 · 每次中期冻结与最终封存必须执行完整审计

- **日期**：2026-09-25
- **决策**：每次里程碑冻结、项目中期冻结和最终封存，必须同时执行内部工程审计、当次官方来源竞品横评、固定范围过滤、仓库清理盘点、静态知识沉淀、演进时间线与归档包生成，并通过测试、链接、安全、状态一致性和远程主干同步门禁。竞品差距只能用于发现既定目标的遗漏，不能自动扩大范围。
- **背景**：单纯的代码测试通过不足以证明项目可封存；计划、实现、界面、文档、竞品现状和演进记录必须能被后续 Agent 独立复原。
- **来源**：用户 2026-09-25 明确要求；[`rules/principles.md`](../../../rules/principles.md) P-002；[2026-09-25 封存审计计划](../../superpowers/plans/active/2026-09-25-project-freeze-closure-audit.md)。
- **状态**：effective。

## Revision History

- 2026-09-25 追加 D-028（中期冻结/最终封存完整审计铁律）。
- 2026-09-17 追加 D-026/D-027（本地单节点产品形态、设计文档同步铁律）。
